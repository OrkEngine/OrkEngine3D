...
namespace BEPUutilities
{
    ...
    public static partial class ConvexHullHelper
    {
        ...
        public static void GetConvexHull(IList<OrkEngine3D.Mathematics.Vector3> points, IList<int> outputTriangleIndices)
        {
        ...
        public static void GetConvexHull(IList<OrkEngine3D.Mathematics.Vector3> points, IList<OrkEngine3D.Mathematics.Vector3> outputSurfacePoints)
        {
            var rawPoints = CommonResources.GetVectorList();
            rawPoints.AddRange(points);
        ...
        /// <param name="points">List of points in the set.</param>
        /// <param name="outputSurfacePoints">Unique points on the surface of the convex hull.</param>
        public static void GetConvexHull(RawList<OrkEngine3D.Mathematics.Vector3> points, IList<OrkEngine3D.Mathematics.Vector3> outputSurfacePoints)
        {
        ...
        /// Each group of 3 indices represents a triangle on the surface of the hull.</param>
        /// <param name="outputSurfacePoints">Unique points on the surface of the convex hull.</param>
        public static void GetConvexHull(IList<OrkEngine3D.Mathematics.Vector3> points, IList<int> outputTriangleIndices, IList<OrkEngine3D.Mathematics.Vector3> outputSurfacePoints)
        {
            var rawPoints = CommonResources.GetVectorList();
            var rawIndices = CommonResources.GetIntList();
            rawPoints.AddRange(points);
        ...
        /// Each group of 3 indices represents a triangle on the surface of the hull.</param>
        /// <param name="outputSurfacePoints">Unique points on the surface of the convex hull.</param>
        public static void GetConvexHull(RawList<OrkEngine3D.Mathematics.Vector3> points, RawList<int> outputTriangleIndices, IList<OrkEngine3D.Mathematics.Vector3> outputSurfacePoints)
        {
        ...
        /// <param name="outputTriangleIndices">List of indices into the input point set composing the triangulated surface of the convex hull.
        /// Each group of 3 indices represents a triangle on the surface of the hull.</param>
        public static void GetConvexHull(RawList<OrkEngine3D.Mathematics.Vector3> points, RawList<int> outputTriangleIndices)
        {
            if (points.Count == 0)
            {
            ...
            while (outsidePoints.Count > 0)
            {
                ...
                for (int k = 0; k < outputTriangleIndices.Count; k += 3)
                {
                    ...
                    OrkEngine3D.Mathematics.Vector3 maximum = points.Elements[maxIndex];

                    //If the point is beyond the current triangle, continue.
                    OrkEngine3D.Mathematics.Vector3 offset;
                    Vector3Ex.Subtract(ref maximum, ref points.Elements[outputTriangleIndices.Elements[k]], out offset);
                    float dot;
        ...
        private static int GetExtremePoint(ref OrkEngine3D.Mathematics.Vector3 direction, RawList<OrkEngine3D.Mathematics.Vector3> points, RawList<int> outsidePoints)
        {
            ...
            {
                float dot;
                Vector3Ex.Dot(ref points.Elements[outsidePoints[i]], ref direction, out dot);
                if (dot > maximumDot)
                {
                    maximumDot = dot;
                    extremeIndex = i;
        ...
        private static void GetExtremePoints(ref OrkEngine3D.Mathematics.Vector3 direction, RawList<OrkEngine3D.Mathematics.Vector3> points, out float maximumDot, out float minimumDot, out int maximumIndex, out int minimumIndex)
        {
            maximumIndex = 0;
            minimumIndex = 0;

            float dot;
            Vector3Ex.Dot(ref points.Elements[0], ref direction, out dot);
            minimumDot = dot;
            maximumDot = dot;
            for (int i = 1; i < points.Count; ++i)
            {
                Vector3Ex.Dot(ref points.Elements[i], ref direction, out dot);
                if (dot > maximumDot)
                {
                    maximumDot = dot;
                    maximumIndex = i;
        ...
        private static void ComputeInitialTetrahedron(RawList<OrkEngine3D.Mathematics.Vector3> points, RawList<int> outsidePointCandidates, RawList<int> triangleIndices, out OrkEngine3D.Mathematics.Vector3 centroid)
        {
            ...
            Vector3Ex.Subtract(ref points.Elements[b], ref points.Elements[a], out ab);
            Vector3Ex.Cross(ref ab, ref Toolbox.UpVector, out direction);
            if (direction.LengthSquared() < Toolbox.Epsilon)
                Vector3Ex.Cross(ref ab, ref Toolbox.RightVector, out direction);
            ...
            float dot;
            Vector3Ex.Dot(ref direction, ref points.Elements[a], out dot);
            ...
            Vector3Ex.Subtract(ref points.Elements[c], ref points.Elements[a], out ac);
            Vector3Ex.Cross(ref ab, ref ac, out direction);
            ...
            Vector3Ex.Add(ref points.Elements[a], ref points.Elements[b], out centroid);
            Vector3Ex.Add(ref centroid, ref points.Elements[c], out centroid);
            Vector3Ex.Add(ref centroid, ref points.Elements[d], out centroid);
            Vector3Ex.Multiply(ref centroid, 0.25f, out centroid);
            ...
            for (int i = 0; i < triangleIndices.Count; i += 3)
            {
                ...
                Vector3Ex.Subtract(ref vB, ref vA, out ab);
                Vector3Ex.Subtract(ref vC, ref vA, out ac);
                Vector3Ex.Cross(ref ac, ref ab, out cross);
                OrkEngine3D.Mathematics.Vector3 offset;
                Vector3Ex.Subtract(ref vA, ref centroid, out offset);
                float volume;
                Vector3Ex.Dot(ref offset, ref cross, out volume);
        ...
        private static void RemoveInsidePoints(RawList<OrkEngine3D.Mathematics.Vector3> points, RawList<int> triangleIndices, RawList<int> outsidePoints)
        {
            ...
            for (int i = 0; i < triangleIndices.Count && insidePoints.Count > 0; i += 3)
            {
                ...
                OrkEngine3D.Mathematics.Vector3 p = points.Elements[triangleIndices.Elements[i]];
                ...
                for (int j = insidePoints.Count - 1; j >= 0; --j)
                {
                    ...
                    OrkEngine3D.Mathematics.Vector3 offset;
                    Vector3Ex.Subtract(ref points.Elements[insidePoints.Elements[j]], ref p, out offset);
                    float dot;
                    Vector3Ex.Dot(ref offset, ref normal, out dot);
        ...
        private static void FindNormal(RawList<int> indices, RawList<OrkEngine3D.Mathematics.Vector3> points, int triangleIndex, out OrkEngine3D.Mathematics.Vector3 normal)
        {
            var a = points.Elements[indices.Elements[triangleIndex]];
            OrkEngine3D.Mathematics.Vector3 ab, ac;
            Vector3Ex.Subtract(ref points.Elements[indices.Elements[triangleIndex + 1]], ref a, out ab);
            Vector3Ex.Subtract(ref points.Elements[indices.Elements[triangleIndex + 2]], ref a, out ac);
            Vector3Ex.Cross(ref ac, ref ab, out normal);
        }

        private static bool IsTriangleVisibleFromPoint(RawList<int> indices, RawList<OrkEngine3D.Mathematics.Vector3> points, int triangleIndex, ref OrkEngine3D.Mathematics.Vector3 point)
        {
            ...
            OrkEngine3D.Mathematics.Vector3 offset;
            Vector3Ex.Subtract(ref point, ref a, out offset);
            float dot;
            Vector3Ex.Dot(ref offset, ref normal, out dot);
            return dot >= 0;
        }

        private static void VerifyWindings(RawList<int> newIndices, RawList<OrkEngine3D.Mathematics.Vector3> points, ref OrkEngine3D.Mathematics.Vector3 centroid)
        {
