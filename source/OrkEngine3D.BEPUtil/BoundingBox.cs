...
namespace BEPUutilities
{
    ...
    public struct BoundingBox
    {
        ...
        /// </summary>
        public OrkEngine3D.Mathematics.Vector3 Min;

        /// <summary>
        /// Location with the highest X, Y, and Z coordinates in the axis-aligned bounding box.
        /// </summary>
        public OrkEngine3D.Mathematics.Vector3 Max;
        ...
        public BoundingBox(OrkEngine3D.Mathematics.Vector3 min, OrkEngine3D.Mathematics.Vector3 max)
        {
            this.Min = min;
            this.Max = max;
        }
        ...
        public OrkEngine3D.Mathematics.Vector3[] GetCorners()
        {
            var toReturn = new OrkEngine3D.Mathematics.Vector3[8];
            toReturn[0] = new OrkEngine3D.Mathematics.Vector3(Min.X, Max.Y, Max.Z);
            toReturn[1] = Max;
            toReturn[2] = new OrkEngine3D.Mathematics.Vector3(Max.X, Min.Y, Max.Z);
            toReturn[3] = new OrkEngine3D.Mathematics.Vector3(Min.X, Min.Y, Max.Z);
            toReturn[4] = new OrkEngine3D.Mathematics.Vector3(Min.X, Max.Y, Min.Z);
            toReturn[5] = new OrkEngine3D.Mathematics.Vector3(Max.X, Max.Y, Min.Z);
            toReturn[6] = new OrkEngine3D.Mathematics.Vector3(Max.X, Min.Y, Min.Z);
            toReturn[7] = Min;
            return toReturn;
        }
        ...
        {
            OrkEngine3D.Mathematics.Vector3 clampedLocation;
            ...
            Vector3Ex.DistanceSquared(ref clampedLocation, ref boundingSphere.Center, out distanceSquared);
        ...
        public static BoundingBox CreateFromPoints(IList<OrkEngine3D.Mathematics.Vector3> points)
        {
            BoundingBox aabb;
            if (points.Count == 0)
            ...
            {
                OrkEngine3D.Mathematics.Vector3 v = points[i];
                if (v.X < aabb.Min.X)
                    aabb.Min.X = v.X;
