    ...
    public class BufferedStatesAccessor
    {
        ...
        public Matrix3x3 OrientationMatrix
        {
            get
            {
                ...
                if (IsReadBufferAccessible())
                {
                    OrkEngine3D.Mathematics.Quaternion o = bufferedStates.BufferedStatesManager.ReadBuffers.GetState(bufferedStates.motionStateIndex).Orientation;
            ...
            set
            {
                if (IsWriteBufferAccessible())
                {
                    OrkEngine3D.Mathematics.Quaternion toSet = QuaternionEx.Normalize(QuaternionEx.CreateFromRotationMatrix(value));
        ...
        public OrkEngine3D.Mathematics.Matrix4x4 WorldTransform
        {
            ...
            set
            {
                if (IsWriteBufferAccessible())
                {
                    OrkEngine3D.Mathematics.Vector3 translation = value.Translation;
