...
namespace BEPUphysics.BroadPhaseEntries
{
    ...
    public class DetectorVolume : BroadPhaseEntry, ISpaceObject, IDeferredEventCreator
    {
        ...
        public bool IsPointContained(OrkEngine3D.Mathematics.Vector3 point)
        {
        ...
        internal bool IsPointContained(ref OrkEngine3D.Mathematics.Vector3 point, RawList<int> triangles)
        {
            ...
            Vector3Ex.Add(ref boundingBox.Max, ref boundingBox.Min, out rayDirection);
            Vector3Ex.Multiply(ref rayDirection, .5f, out rayDirection);
            Vector3Ex.Subtract(ref point, ref rayDirection, out rayDirection);
            //If the point is right in the middle, we'll need a backup.
            if (rayDirection.LengthSquared() < .01f)
                rayDirection = Vector3Ex.Up;
        ...
        public override bool ConvexCast(ConvexShape castShape, ref RigidTransform startingTransform, ref OrkEngine3D.Mathematics.Vector3 sweep, out RayHit hit)
        {
            hit = new RayHit();
            ...
            if (triangleMesh.Tree.GetOverlaps(boundingBox, hitElements))
            {
                ...
                for (int i = 0; i < hitElements.Count; i++)
                {
                    ...
                    var triangleTransform = new RigidTransform { Orientation = OrkEngine3D.Mathematics.Quaternion.Identity, Position = center };
                    RayHit tempHit;
        ...
        public void Reinitialize()
        {
            ...
            OrkEngine3D.Mathematics.Vector3 origin = (triangleMesh.Tree.BoundingBox.Max - triangleMesh.Tree.BoundingBox.Min) * 1.5f + triangleMesh.Tree.BoundingBox.Min;
