...
namespace BEPUphysics.BroadPhaseEntries.MobileCollidables
{
    ...
    public class MobileMeshCollidable : EntityCollidable
    {
        ...
        public override bool RayCast(Ray ray, float maximumLength, out RayHit rayHit)
        {
            ...
            Vector3Ex.Subtract(ref ray.Position, ref worldTransform.Position, out localRay.Position);
            ...
            if (Shape.solidity == MobileMeshSolidity.Solid)
            {
                ...
                {
                    //It was inside!
                    rayHit = new RayHit() { Location = ray.Position, Normal = OrkEngine3D.Mathematics.Vector3.Zero, T = 0 };
                    return true;
                ...
                else
                {
                    if (rayHit.T < maximumLength)
                    {
                        ...
                        Vector3Ex.Multiply(ref ray.Direction, rayHit.T, out rayHit.Location);
                        Vector3Ex.Add(ref rayHit.Location, ref ray.Position, out rayHit.Location);
        ...
        public override bool ConvexCast(ConvexShape castShape, ref RigidTransform startingTransform, ref OrkEngine3D.Mathematics.Vector3 sweep, out RayHit hit)
        {
            if (Shape.solidity == MobileMeshSolidity.Solid)
            {
                ...
                if (Shape.IsLocalRayOriginInMesh(ref ray, out hit))
                {
                    ...
                    hit = new RayHit() { Location = startingTransform.Position, Normal = new OrkEngine3D.Mathematics.Vector3(), T = 0 };
                    return true;
                }
            }
            hit = new RayHit();
