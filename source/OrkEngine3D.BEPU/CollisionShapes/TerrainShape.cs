...
namespace BEPUphysics.CollisionShapes
{
    ...
    public class TerrainShape : CollisionShape
    {
        ...
        public void GetBoundingBox(ref AffineTransform transform, out BoundingBox boundingBox)
        {
            ...
            OrkEngine3D.Mathematics.Vector3 minXvertex = new OrkEngine3D.Mathematics.Vector3(),
                    maxXvertex = new OrkEngine3D.Mathematics.Vector3(),
                    minYvertex = new OrkEngine3D.Mathematics.Vector3(),
                    maxYvertex = new OrkEngine3D.Mathematics.Vector3(),
                    minZvertex = new OrkEngine3D.Mathematics.Vector3(),
                    maxZvertex = new OrkEngine3D.Mathematics.Vector3();
            ...
            for (int i = 0; i < heights.GetLength(0); i++)
            {
                ...
                {
                    var vertex = new OrkEngine3D.Mathematics.Vector3(i, heights[i, j], j);
        ...
        public bool RayCast(ref Ray ray, float maximumLength, ref AffineTransform transform, TriangleSidedness sidedness, out RayHit hit)
        {
            ...
            OrkEngine3D.Mathematics.Vector3 progressingOrigin = localRay.Position;
            float distance = 0;
            ...
            if (progressingOrigin.X < 0)
            {
                if (localRay.Direction.X > 0)
                {
                    ...
                    distance += timeToMinX;
                    OrkEngine3D.Mathematics.Vector3 increment;
            ...
            while (true)
            {
                ...
                OrkEngine3D.Mathematics.Vector3 v1, v2, v3, v4;
                // v3 v4
                // v1 v2
        ...
        public void GetLocalPosition(int columnIndex, int rowIndex, out OrkEngine3D.Mathematics.Vector3 v)
        {
#if !WINDOWS
            v = new OrkEngine3D.Mathematics.Vector3();
#endif
            v.X = columnIndex;
        ...
        public void GetPosition(int columnIndex, int rowIndex, ref AffineTransform transform, out OrkEngine3D.Mathematics.Vector3 position)
        {
            if (columnIndex <= 0)
                columnIndex = 0;
        ...
        public void GetLocalNormal(int columnIndex, int rowIndex, out OrkEngine3D.Mathematics.Vector3 normal)
        {
            ...
            //Since the horizontal offsets are known to be 1 in local space, we can omit quite a few operations compared to a full OrkEngine3D.Mathematics.Vector3 and cross product.

            //The implicit vectors are:
            //var leftToRight = new OrkEngine3D.Mathematics.Vector3(2, rightHeight - leftHeight, 0);
            //var bottomToTop = new OrkEngine3D.Mathematics.Vector3(0, topHeight - bottomHeight, 2);
            //the result is then:
            //Vector3Ex.Cross(bottomToTop, leftToRight);
            //Which is:
        ...
        public void GetFirstTriangle(int columnIndex, int rowIndex, ref AffineTransform transform, out OrkEngine3D.Mathematics.Vector3 a, out OrkEngine3D.Mathematics.Vector3 b, out OrkEngine3D.Mathematics.Vector3 c)
        {
            if (quadTriangleOrganization == QuadTriangleOrganization.BottomLeftUpperRight)
            {
        ...
        ///<param name="b">Second vertex of the triangle.</param>
        ///<param name="c">Third vertex of the triangle.</param>
        public void GetSecondTriangle(int columnIndex, int rowIndex, ref AffineTransform transform, out OrkEngine3D.Mathematics.Vector3 a, out OrkEngine3D.Mathematics.Vector3 b, out OrkEngine3D.Mathematics.Vector3 c)
        {
            if (quadTriangleOrganization == QuadTriangleOrganization.BottomLeftUpperRight)
            {
        ...
        ///<param name="b">Second vertex of the triangle.</param>
        ///<param name="c">Third vertex of the triangle.</param>
        public void GetTriangle(int index, ref AffineTransform transform, out OrkEngine3D.Mathematics.Vector3 a, out OrkEngine3D.Mathematics.Vector3 b, out OrkEngine3D.Mathematics.Vector3 c)
        {
            //Find the quad.
