...
namespace BEPUphysics.CollisionShapes
{
    ...
    public class MobileMeshShape : EntityShape
    {
        ...
        RawList<OrkEngine3D.Mathematics.Vector3> hullVertices = new RawList<OrkEngine3D.Mathematics.Vector3>();

        /// <summary>
        /// Gets the list of vertices on the convex hull of the mesh used to compute the bounding box.
        /// </summary>
        public ReadOnlyList<OrkEngine3D.Mathematics.Vector3> HullVertices
        {
            get { return new ReadOnlyList<OrkEngine3D.Mathematics.Vector3>(hullVertices); }
        }
        ...
        public MobileMeshShape(OrkEngine3D.Mathematics.Vector3[] vertices, int[] indices, AffineTransform localTransform, MobileMeshSolidity solidity)
        {
        ...
        public MobileMeshShape(OrkEngine3D.Mathematics.Vector3[] vertices, int[] indices, AffineTransform localTransform, MobileMeshSolidity solidity, out OrkEngine3D.Mathematics.Vector3 center)
        {
            this.solidity = solidity;
            var data = new TransformableMeshData(vertices, indices, localTransform);
            var shapeDistributionInformation = ComputeVolumeDistribution(data);
        ...
        public MobileMeshShape(TransformableMeshData meshData, IList<OrkEngine3D.Mathematics.Vector3> hullVertices, MobileMeshSolidity solidity, TriangleSidedness sidednessWhenSolid, float collisionMargin, EntityShapeVolumeDescription volumeDescription)
        {
            triangleMesh = new TriangleMesh(meshData);
            this.hullVertices = new RawList<OrkEngine3D.Mathematics.Vector3>(hullVertices);
        ...
        public bool IsLocalRayOriginInMesh(ref Ray ray, out RayHit hit)
        {
            ...
            if (triangleMesh.Tree.GetOverlaps(ray, overlapList))
            {
                ...
                {
                    OrkEngine3D.Mathematics.Vector3 vA, vB, vC;
        ...
        TriangleSidedness ComputeSolidSidednessHelper(Ray ray)
        {
            ...
            if (triangleMesh.Tree.GetOverlaps(ray, hitList))
            {
                ...
                if (hits.Count % 2 == 0)
                {
                    ...
                    var normal = OrkEngine3D.Mathematics.Vector3.Cross(vA - vB, vA - vC);
                    if (Vector3Ex.Dot(normal, ray.Direction) < 0)
        ...
        private void UpdateSurfaceVertices()
        {
            ...
            else
            {
                ...
                hullVertices.Add(new OrkEngine3D.Mathematics.Vector3(box.Min.X, box.Min.Y, box.Max.Z));
                hullVertices.Add(new OrkEngine3D.Mathematics.Vector3(box.Min.X, box.Max.Y, box.Min.Z));
                hullVertices.Add(new OrkEngine3D.Mathematics.Vector3(box.Max.X, box.Min.Y, box.Min.Z));
                hullVertices.Add(new OrkEngine3D.Mathematics.Vector3(box.Min.X, box.Max.Y, box.Max.Z));
                hullVertices.Add(new OrkEngine3D.Mathematics.Vector3(box.Max.X, box.Max.Y, box.Min.Z));
                hullVertices.Add(new OrkEngine3D.Mathematics.Vector3(box.Max.X, box.Min.Y, box.Max.Z));
            }
        }
        ...
        private ShapeDistributionInformation ComputeVolumeDistribution(TransformableMeshData data)
        {
            ...
            shapeInformation.Center = new OrkEngine3D.Mathematics.Vector3();
            ...
            for (int i = 0; i < data.indices.Length; i += 3)
            {
                ...
                OrkEngine3D.Mathematics.Vector3 vAvB;
                OrkEngine3D.Mathematics.Vector3 vAvC;
        ...
        private void GetBoundingBox(ref Matrix3x3 o, out BoundingBox boundingBox)
        {
            ...
            var rightDirection = new OrkEngine3D.Mathematics.Vector3(o.M11, o.M21, o.M31);
            var upDirection = new OrkEngine3D.Mathematics.Vector3(o.M12, o.M22, o.M32);
            var backDirection = new OrkEngine3D.Mathematics.Vector3(o.M13, o.M23, o.M33);
        ...
        /// <param name="boundingBox">Bounding box in the local space.</param>
        public void GetSweptLocalBoundingBox(ref RigidTransform shapeTransform, ref AffineTransform spaceTransform, ref OrkEngine3D.Mathematics.Vector3 sweep, out BoundingBox boundingBox)
        {
            GetLocalBoundingBox(ref shapeTransform, ref spaceTransform, out boundingBox);
            OrkEngine3D.Mathematics.Vector3 expansion;
