...
namespace BEPUphysics.CollisionShapes.ConvexShapes
{
    ...
    public static class InertiaHelper
    {
        ...
        public static void GetPointContribution(float pointWeight, ref OrkEngine3D.Mathematics.Vector3 center, ref OrkEngine3D.Mathematics.Vector3 point, out Matrix3x3 contribution)
        {
            Vector3Ex.Subtract(ref point, ref center, out point);
        ...
        /// </summary>
        public static OrkEngine3D.Mathematics.Vector3[] SampleDirections;
        ...
        public static void GenerateSphere(int subdivisionCount, out OrkEngine3D.Mathematics.Vector3[] vertices, out int[] triangles)
        {
            ...
            vertices = new OrkEngine3D.Mathematics.Vector3[GetExpectedVertexCount(subdivisionCount)];

            ////Create the regular tetrahedron vertices.
            //float x = (float)(1 / Math.Sqrt(3));
            //float z = (float)(-1 / (2 * Math.Sqrt(6)));
            //vertices[0] = OrkEngine3D.Mathematics.Vector3.Normalize(new OrkEngine3D.Mathematics.Vector3(0, 0, (float)(Math.Sqrt(2.0 / 3.0) + z)));
            //vertices[1] = OrkEngine3D.Mathematics.Vector3.Normalize(new OrkEngine3D.Mathematics.Vector3(-0.5f * x, -0.5f, z));
            //vertices[2] = OrkEngine3D.Mathematics.Vector3.Normalize(new OrkEngine3D.Mathematics.Vector3(-0.5f * x, 0.5f, z));
            //vertices[3] = OrkEngine3D.Mathematics.Vector3.Normalize(new OrkEngine3D.Mathematics.Vector3(x, 0, z));
            ...
            //OrkEngine3D.Mathematics.Vector3[] vertices = new OrkEngine3D.Mathematics.Vector3[12];
            var goldenRatio = (1 + (float)Math.Sqrt(5)) / 2;
            float length = (float)Math.Sqrt(1 + goldenRatio * goldenRatio);
            float x = 1 / length;
            float y = goldenRatio / length;
            vertices[0] = new OrkEngine3D.Mathematics.Vector3(0, x, y);
            vertices[1] = new OrkEngine3D.Mathematics.Vector3(0, -x, y);
            vertices[2] = new OrkEngine3D.Mathematics.Vector3(0, x, -y);
            vertices[3] = new OrkEngine3D.Mathematics.Vector3(0, -x, -y);

            vertices[4] = new OrkEngine3D.Mathematics.Vector3(x, y, 0);
            vertices[5] = new OrkEngine3D.Mathematics.Vector3(-x, y, 0);
            vertices[6] = new OrkEngine3D.Mathematics.Vector3(x, -y, 0);
            vertices[7] = new OrkEngine3D.Mathematics.Vector3(-x, -y, 0);

            vertices[8] = new OrkEngine3D.Mathematics.Vector3(y, 0, x);
            vertices[9] = new OrkEngine3D.Mathematics.Vector3(-y, 0, x);
            vertices[10] = new OrkEngine3D.Mathematics.Vector3(y, 0, -x);
            vertices[11] = new OrkEngine3D.Mathematics.Vector3(-y, 0, -x);
            ...
            for (int i = 0; i < subdivisionCount; ++i)
            {
                for (int triangleIndex = 0; triangleIndex < currentTriangleIndexCount; triangleIndex += 3)
                {
                    ...
                    if (!edges.TryGetValue(edge, out abMidIndex))
                    {
                        ...
                        //Create a vertex.
                        OrkEngine3D.Mathematics.Vector3 mid;
            ...
            //RawList<OrkEngine3D.Mathematics.Vector3> surfacePoints = new RawList<OrkEngine3D.Mathematics.Vector3>();
        ...
        public static void ComputeShapeDistribution(IList<OrkEngine3D.Mathematics.Vector3> vertices, IList<int> triangleIndices, out float volume, out Matrix3x3 volumeDistribution)
        {
            ...
            {
                OrkEngine3D.Mathematics.Vector3 v2 = vertices[triangleIndices[i]];
                OrkEngine3D.Mathematics.Vector3 v3 = vertices[triangleIndices[i + 1]];
                OrkEngine3D.Mathematics.Vector3 v4 = vertices[triangleIndices[i + 2]];
        ...
        /// <param name="volume">Volume of the shape.</param>
        /// <param name="volumeDistribution">Distribution of the volume as measured from the computed center.</param>
        public static void ComputeShapeDistribution(IList<OrkEngine3D.Mathematics.Vector3> vertices, IList<int> triangleIndices, out OrkEngine3D.Mathematics.Vector3 center, out float volume, out Matrix3x3 volumeDistribution)
        {
            //Explanation for the tetrahedral integration bits: Explicit Exact Formulas for the 3-D Tetrahedron Inertia Tensor in Terms of its Vertex Coordinates
            //http://www.scipub.org/fulltext/jms2/jms2118-11.pdf
            //x1, x2, x3, x4 are origin, triangle1, triangle2, triangle3
            ...
            OrkEngine3D.Mathematics.Vector3 summedCenter = new OrkEngine3D.Mathematics.Vector3();
            float scaledVolume = 0;
            for (int i = 0; i < triangleIndices.Count; i += 3)
            {
                OrkEngine3D.Mathematics.Vector3 v2 = vertices[triangleIndices[i]];
                ...
                OrkEngine3D.Mathematics.Vector3 tetrahedronCentroid;
                Vector3Ex.Add(ref v2, ref v3, out tetrahedronCentroid);
                Vector3Ex.Add(ref tetrahedronCentroid, ref v4, out tetrahedronCentroid);
                Vector3Ex.Multiply(ref tetrahedronCentroid, scaledTetrahedronVolume, out tetrahedronCentroid);
                Vector3Ex.Add(ref tetrahedronCentroid, ref summedCenter, out summedCenter);
            ...
            if (scaledVolume < Toolbox.Epsilon)
            {
                ...
                volume = 0;
                center = new OrkEngine3D.Mathematics.Vector3();
            }
            else
            {
                Vector3Ex.Multiply(ref summedCenter, 0.25f / scaledVolume, out center);
                volume = scaledVolume / 6;
                float scaledDensity = 1 / volume;
                float diagonalFactor = scaledDensity / 60;
                float offFactor = -scaledDensity / 120;
        ...
        //public static void GetInertiaOffset(OrkEngine3D.Mathematics.Vector3 offset, float mass, out Matrix3x3 additionalInertia)
        //{
        ...
        public static float ComputeMinimumRadius(IList<OrkEngine3D.Mathematics.Vector3> vertices, IList<int> triangleIndices, ref OrkEngine3D.Mathematics.Vector3 center)
        {
            ...
            for (int i = 0; i < triangleIndices.Count; i += 3)
            {
                ...
                OrkEngine3D.Mathematics.Vector3 v2v3, v2v4;
                ...
                OrkEngine3D.Mathematics.Vector3 fromCenterToPlane;
