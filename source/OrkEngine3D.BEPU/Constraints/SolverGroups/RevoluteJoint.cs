...
namespace BEPUphysics.Constraints.SolverGroups
{
    ...
    public class RevoluteJoint : SolverGroup
    {
        ...
        public RevoluteJoint(Entity connectionA, Entity connectionB, OrkEngine3D.Mathematics.Vector3 anchor, OrkEngine3D.Mathematics.Vector3 freeAxis)
        {
            if (connectionA == null)
            ...
            if (baseAxis.LengthSquared() < Toolbox.BigEpsilon)
            {
                ...
                baseAxis = OrkEngine3D.Mathematics.Vector3.Cross(freeAxis, Vector3Ex.Up);
                if (baseAxis.LengthSquared() < Toolbox.BigEpsilon)
                {
                    baseAxis = OrkEngine3D.Mathematics.Vector3.Cross(freeAxis, Vector3Ex.Right);
                }
            }
