...
namespace BEPUphysics.Constraints.TwoEntity.JointLimits
{
    ...
    public class RevoluteLimit : JointLimit, I2DImpulseConstraintWithError, I2DJacobianConstraint
    {
        ...
        private OrkEngine3D.Mathematics.Vector3 jacobianMaxA;
        private OrkEngine3D.Mathematics.Vector3 jacobianMaxB;
        private OrkEngine3D.Mathematics.Vector3 jacobianMinA;
        private OrkEngine3D.Mathematics.Vector3 jacobianMinB;
        private bool maxIsActive;
        private bool minIsActive;
        private OrkEngine3D.Mathematics.Vector2 error;
        private OrkEngine3D.Mathematics.Vector3 localTestAxis;
        ...
        private OrkEngine3D.Mathematics.Vector3 worldTestAxis;
        private OrkEngine3D.Mathematics.Vector2 velocityToImpulse;
        ...
        public RevoluteLimit(Entity connectionA, Entity connectionB, OrkEngine3D.Mathematics.Vector3 limitedAxis, OrkEngine3D.Mathematics.Vector3 testAxis, float minimumAngle, float maximumAngle)
        {
        ...
        /// </summary>
        public OrkEngine3D.Mathematics.Vector3 LocalTestAxis
        {
            get { return localTestAxis; }
            set
            {
                localTestAxis = OrkEngine3D.Mathematics.Vector3.Normalize(value);
        ...
        /// </summary>
        public OrkEngine3D.Mathematics.Vector3 TestAxis
        {
            get { return worldTestAxis; }
            set
            {
                worldTestAxis = OrkEngine3D.Mathematics.Vector3.Normalize(value);
        ...
        public OrkEngine3D.Mathematics.Vector2 RelativeVelocity
        {
            get
            {
                if (isLimitActive)
                {
                    ...
                    OrkEngine3D.Mathematics.Vector2 toReturn = OrkEngine3D.Mathematics.Vector2.Zero;
                    if (minIsActive)
                    {
                        Vector3Ex.Dot(ref connectionA.angularVelocity, ref jacobianMinA, out velocityA);
                        Vector3Ex.Dot(ref connectionB.angularVelocity, ref jacobianMinB, out velocityB);
                        toReturn.X = velocityA + velocityB;
                    }
                    if (maxIsActive)
                    {
                        Vector3Ex.Dot(ref connectionA.angularVelocity, ref jacobianMaxA, out velocityA);
                        Vector3Ex.Dot(ref connectionB.angularVelocity, ref jacobianMaxB, out velocityB);
        ...
        public override float SolveIteration()
        {
            ...
            if (minIsActive)
            {
                ...
                {
                    Vector3Ex.Multiply(ref jacobianMinA, lambda, out impulse);
                    connectionA.ApplyAngularImpulse(ref impulse);
                }
                if (connectionB.isDynamic)
                {
                    Vector3Ex.Multiply(ref jacobianMinB, lambda, out impulse);
            ...
            if (maxIsActive)
            {
                ...
                if (connectionA.isDynamic)
                {
                    Vector3Ex.Multiply(ref jacobianMaxA, lambda, out impulse);
                    connectionA.ApplyAngularImpulse(ref impulse);
                }
                if (connectionB.isDynamic)
                {
                    Vector3Ex.Multiply(ref jacobianMaxB, lambda, out impulse);
                    connectionB.ApplyAngularImpulse(ref impulse);
                }
        ...
        public override void Update(float dt)
        {
            ...
            OrkEngine3D.Mathematics.Vector3 minPlaneNormal, maxPlaneNormal;
            ...
            if (span >= MathHelper.Pi)
            {
                if (planePositionMax > 0 || planePositionMin > 0)
                {
                    ...
                    minIsActive = false;
                    maxIsActive = false;
                    error = OrkEngine3D.Mathematics.Vector2.Zero;
                    accumulatedImpulse = OrkEngine3D.Mathematics.Vector2.Zero;
                    isLimitActive = false;
                    return;
                }
            ...
            OrkEngine3D.Mathematics.Vector3 transformedAxis;
            if (connectionA.isDynamic)
            {
                if (minIsActive)
                {
        ...
        public override void ExclusiveUpdate()
        {
            ...
            if (connectionA.isDynamic)
            {
                var impulse = new OrkEngine3D.Mathematics.Vector3();
                if (minIsActive)
                {
