...
namespace BEPUphysics.CollisionTests.CollisionAlgorithms
{
    ...
    public static class MPRToolbox
    {
        ...
        public static bool GetOverlapPosition(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform transformA, ref RigidTransform transformB, out OrkEngine3D.Mathematics.Vector3 position)
        {
        ...
        /// <returns>Whether or not the two shapes overlap.</returns>
        public static bool GetLocalOverlapPosition(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform localTransformB, out OrkEngine3D.Mathematics.Vector3 position)
        {
            return GetLocalOverlapPosition(shapeA, shapeB, ref localTransformB.Position, ref localTransformB, out position);
        }

        internal static bool GetLocalOverlapPosition(ConvexShape shapeA, ConvexShape shapeB, ref OrkEngine3D.Mathematics.Vector3 originRay, ref RigidTransform localTransformB, out OrkEngine3D.Mathematics.Vector3 position)
        {
            ...
            {
                position = new OrkEngine3D.Mathematics.Vector3();
                //DEBUGlastPosition = position;
                return true;
            }

            OrkEngine3D.Mathematics.Vector3 v0;
            ...
            OrkEngine3D.Mathematics.Vector3 n = originRay;
            OrkEngine3D.Mathematics.Vector3 v1;
            OrkEngine3D.Mathematics.Vector3 v1A, v1B; //extreme point contributions from each shape.  Used later to compute contact position; could be used to cache simplex too.
            ...
            OrkEngine3D.Mathematics.Vector3 v2;
            OrkEngine3D.Mathematics.Vector3 v2A, v2B;
            ...
            OrkEngine3D.Mathematics.Vector3 temp1, temp2;
            ...
            OrkEngine3D.Mathematics.Vector3 v3A, v3B, v3;
            int count = 0;
            while (true)
            {
            ...
            while (true)
            {
                ...
                if (dot >= 0)
                {
                    OrkEngine3D.Mathematics.Vector3 temp3;
                ...
                OrkEngine3D.Mathematics.Vector3 v4, v4A, v4B;
        ...
        /// <param name="localTransformB">Relative transform of shape B to shape A.</param>
        /// <returns>Whether or not the shapes are overlapping.</returns>
        internal static bool AreLocalShapesOverlapping(ConvexShape shapeA, ConvexShape shapeB, ref OrkEngine3D.Mathematics.Vector3 originRay, ref RigidTransform localTransformB)
        {
            //Compute the origin ray.  This points from a point known to be inside the minkowski sum to the origin.
            //The centers of the shapes are used to create the interior point.
            ...
            OrkEngine3D.Mathematics.Vector3 v3;
            int count = 0;
            while (true)
            {
                //Find a final extreme point using the normal of the plane defined by v0, v1, v2.
            ...
            while (true)
            {
                ...
                //We haven't yet found the origin.  Find the support point in the portal's outward facing direction.
                OrkEngine3D.Mathematics.Vector3 v4;
        ...
        public static void LocalSurfaceCast(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform localTransformB, ref OrkEngine3D.Mathematics.Vector3 direction, out float t, out OrkEngine3D.Mathematics.Vector3 normal)
        {
            ...
            //This implementation is similar to that of the original XenoCollide.
            //'n' will be the direction used to find supports throughout the algorithm.
            OrkEngine3D.Mathematics.Vector3 n = direction;
            OrkEngine3D.Mathematics.Vector3 v1;
            MinkowskiToolbox.GetLocalMinkowskiExtremePoint(shapeA, shapeB, ref n, ref localTransformB, out v1);
            ...
            if (n.LengthSquared() < Toolbox.Epsilon)
            {
                ...
                if (rayLengthSquared > Toolbox.Epsilon * .01f)
                    ...
                    normal = new OrkEngine3D.Mathematics.Vector3();
        ...
        public static void LocalSurfaceCast(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform localTransformB, ref OrkEngine3D.Mathematics.Vector3 direction, out float t, out OrkEngine3D.Mathematics.Vector3 normal, out OrkEngine3D.Mathematics.Vector3 position)
        {
            // Local surface cast is very similar to regular MPR.  However, instead of starting at an interior point and targeting the origin,
            // the ray starts at the origin (a point known to be in both shape and shapeB), and just goes towards the direction until the surface
            // is found.  The portal (v1, v2, v3) at termination defines the surface normal, and the distance from the origin to the portal along the direction is used as the 't' result.
            ...
            //'n' will be the direction used to find supports throughout the algorithm.
            OrkEngine3D.Mathematics.Vector3 n = direction;
            OrkEngine3D.Mathematics.Vector3 v1, v1A;
            ...
            //Find another extreme point in a direction perpendicular to the previous.
            OrkEngine3D.Mathematics.Vector3 v2, v2A;
            Vector3Ex.Cross(ref direction, ref v1, out n);
            if (n.LengthSquared() < Toolbox.Epsilon)
            {
                //v1 and v0 could be parallel.
            ...
            OrkEngine3D.Mathematics.Vector3 v3, v3A;
            int count = 0;
            while (true)
            {
                //Find a final extreme point using the normal of the plane defined by v0, v1, v2.
            ...
            while (true)
            {
                ...
                //Keep working towards the surface.  Find the next extreme point.
                OrkEngine3D.Mathematics.Vector3 v4, v4A;
        ...
        static bool VerifySimplex(ref OrkEngine3D.Mathematics.Vector3 v0, ref OrkEngine3D.Mathematics.Vector3 v1, ref OrkEngine3D.Mathematics.Vector3 v2, ref OrkEngine3D.Mathematics.Vector3 v3, ref OrkEngine3D.Mathematics.Vector3 direction)
        {


            //v1, v0, v3
            OrkEngine3D.Mathematics.Vector3 cross = OrkEngine3D.Mathematics.Vector3.Cross(v0 - v1, v3 - v1);
            float planeProduct1 = Vector3Ex.Dot(cross, direction);
            //v3, v0, v2
            cross = OrkEngine3D.Mathematics.Vector3.Cross(v0 - v3, v2 - v3);
            float planeProduct2 = Vector3Ex.Dot(cross, direction);
            //v2, v0, v1
            cross = OrkEngine3D.Mathematics.Vector3.Cross(v0 - v2, v1 - v2);
            float planeProduct3 = Vector3Ex.Dot(cross, direction);
        ...
        public static bool GetContact(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform transformA, ref RigidTransform transformB, ref OrkEngine3D.Mathematics.Vector3 penetrationAxis, out ContactData contact)
        {
            RigidTransform localTransformB;
            MinkowskiToolbox.GetLocalTransform(ref transformA, ref transformB, out localTransformB);
            if (MPRToolbox.AreLocalShapesOverlapping(shapeA, shapeB, ref localTransformB))
            {
                ...
                OrkEngine3D.Mathematics.Vector3 rayCastDirection;
                float lengthSquared = penetrationAxis.LengthSquared();
                if (lengthSquared > Toolbox.Epsilon)
                {
                    Vector3Ex.Divide(ref penetrationAxis, (float)Math.Sqrt(lengthSquared), out rayCastDirection);// (OrkEngine3D.Mathematics.Vector3.Normalize(localDirection) + OrkEngine3D.Mathematics.Vector3.Normalize(collidableB.worldTransform.Position - collidableA.worldTransform.Position)) / 2;
                ...
                OrkEngine3D.Mathematics.Vector3 normalCandidate;
        ...
        public static void RefinePenetration(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform localTransformB, float initialDepth, ref OrkEngine3D.Mathematics.Vector3 initialNormal, out float penetrationDepth, out OrkEngine3D.Mathematics.Vector3 refinedNormal, out OrkEngine3D.Mathematics.Vector3 position)
        {
            //The local casting can optionally continue.  Eventually, it will converge to the local minimum.
            ...
            OrkEngine3D.Mathematics.Vector3 candidateNormal;
        ...
        public static bool Sweep(ConvexShape shapeA, ConvexShape shapeB, ref OrkEngine3D.Mathematics.Vector3 sweepA, ref OrkEngine3D.Mathematics.Vector3 sweepB, ref RigidTransform transformA, ref RigidTransform transformB, out RayHit hit)
        {
            //Put the relative velocity into shape's local space.
            OrkEngine3D.Mathematics.Vector3 velocityWorld;
            //note the order of subtraction.  It 'should' be B-A, but the ray direction the algorithm works with is actually OPPOSITE.
            Vector3Ex.Subtract(ref sweepA, ref sweepB, out velocityWorld);
            ...
            OrkEngine3D.Mathematics.Vector3 localDirection;
            QuaternionEx.Transform(ref velocityWorld, ref conjugateOrientationA, out localDirection);
            ...
            if (rayLengthSquared > Toolbox.Epsilon * .01f)
            {
                Vector3Ex.Dot(ref localTransformB.Position, ref localDirection, out sweepLength);
            ...
            Vector3Ex.Multiply(ref localDirection, sweepLength, out sweep);
            ...
            if (!AreSweptShapesIntersecting(shapeA, shapeB, ref sweep, ref localTransformB, out hit.Location)) //Computes a hit location to be used if the early-outs due to being in contact.
            {
                ...
                hit.Location = new OrkEngine3D.Mathematics.Vector3();
                return false;
            }
            if (negativeLength)
            {
                ...
                hit.T = 0;
                Vector3Ex.Normalize(ref localDirection, out hit.Normal);
                QuaternionEx.Transform(ref hit.Normal, ref transformA.Orientation, out hit.Normal);
                //hit.Location = hit.T * localDirection;
                QuaternionEx.Transform(ref hit.Location, ref transformA.Orientation, out hit.Location);
            ...
            if (LocalSweepCast(shapeA, shapeB, sweepLength, rayLengthSquared, ref localDirection, ref sweep, ref localTransformB, out hit))
            {
                ...
                OrkEngine3D.Mathematics.Vector3 minkowskiRayHit = -hit.T * localDirection;
                ...
                OrkEngine3D.Mathematics.Vector3 temp;
                Vector3Ex.Multiply(ref sweepA, hit.T, out temp);
                Vector3Ex.Add(ref temp, ref hit.Location, out hit.Location);
                return true;
            }
            return false;
        ...
        private static bool LocalSweepCast(ConvexShape shape, ConvexShape shapeB, float sweepLength, float rayLengthSquared, ref OrkEngine3D.Mathematics.Vector3 localDirection, ref OrkEngine3D.Mathematics.Vector3 sweep, ref RigidTransform localTransformB, out RayHit hit)
        {
            ...
            //This implementation is similar to that of the original XenoCollide.
            //'n' will be the direction used to find supports throughout the algorithm.
            OrkEngine3D.Mathematics.Vector3 n = localDirection;
            OrkEngine3D.Mathematics.Vector3 v1, v1A;
            ...
            //{
            //    hit.T = 0;
            //    OrkEngine3D.Mathematics.Vector3.Normalize(ref n, out hit.Normal);
            //    OrkEngine3D.Mathematics.Vector3.Transform(ref hit.Normal, ref transform.Orientation, out hit.Normal);
            //    //hit.Location = hit.T * localDirection;
            //    OrkEngine3D.Mathematics.Vector3.Transform(ref hit.Location, ref transform.Orientation, out hit.Location);
            //    Vector3Ex.Add(ref hit.Location, ref transform.Position, out hit.Location);
            //    hit.Location += sweepA * hit.T;
            //    return true;
            //}
            ...
            //Find another extreme point in a direction perpendicular to the previous.
            OrkEngine3D.Mathematics.Vector3 v2, v2A;
            Vector3Ex.Cross(ref localDirection, ref v1, out n);
            hit.Location = new OrkEngine3D.Mathematics.Vector3();
            if (n.LengthSquared() < Toolbox.Epsilon * .01f)
            {
                ...
                if (rayLengthSquared > Toolbox.Epsilon * .01f)
                    Vector3Ex.Divide(ref localDirection, (float)Math.Sqrt(rayLengthSquared), out hit.Normal);
                else
                    hit.Normal = new OrkEngine3D.Mathematics.Vector3();

                float rate;
                Vector3Ex.Dot(ref  hit.Normal, ref localDirection, out rate);
                float distance;
                Vector3Ex.Dot(ref  hit.Normal, ref v1, out distance);
                if (rate > 0)
            ...
            //relative to the direction vector.
            float dot;
            Vector3Ex.Dot(ref n, ref localDirection, out dot);
            if (dot > 0)
            {
                //It's not properly calibrated.  Flip the winding (and the previously calculated normal).
                Vector3Ex.Negate(ref n, out n);
            ...
            while (true)
            {
                ...
                // If the direction is outside the plane defined by v1,v0,v3, then the portal is invalid.
                Vector3Ex.Cross(ref v1, ref v3, out temp1);
                Vector3Ex.Dot(ref temp1, ref localDirection, out dot);
                if (dot < 0)
                {
                    //Replace the point that was on the inside of the plane (v2) with the new extreme point.
                    v2 = v3;
            ...
            while (true)
            {
                ...
                if (supportDot - dot < rayCastSurfaceEpsilon || count > MPRToolbox.InnerIterationLimit) // TODO: Could use a dynamic epsilon for possibly better behavior.
                {
                    ...
                    {
                        Vector3Ex.Divide(ref n, (float)Math.Sqrt(lengthSquared), out hit.Normal);

                        //The plane is very close to the surface, and the ray is known to pass through it.
                        //dot is the rate.
                        Vector3Ex.Dot(ref  hit.Normal, ref localDirection, out dot);
                        //supportDot is the distance to the plane.
                        Vector3Ex.Dot(ref  hit.Normal, ref v1, out supportDot);
                    ...
                    //OrkEngine3D.Mathematics.Vector3 mdHitLocation = t * localDirection;
                    //float v1Weight, v2Weight, v3Weight;
                ...
                //This v4 x direction is just a minor reordering of a scalar triple product: (v1 x v4) * direction.
                //It eliminates the need for extra cross products for the inner if.
                Vector3Ex.Cross(ref v4, ref localDirection, out temp1);
                Vector3Ex.Dot(ref v1, ref temp1, out dot);
                if (dot >= 0)
                {
                    Vector3Ex.Dot(ref v2, ref temp1, out dot);
        ...
        internal static void GetLocalPosition(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform localTransformB, ref OrkEngine3D.Mathematics.Vector3 minkowskiPosition, out OrkEngine3D.Mathematics.Vector3 position)
        {
            //Compute the ray.  This points from a point known to be inside the minkowski sum to the test minkowski position;
            //The centers of the shapes are used to create the interior point.
            OrkEngine3D.Mathematics.Vector3 rayDirection;
            Vector3Ex.Add(ref minkowskiPosition, ref localTransformB.Position, out rayDirection);
            ...
            OrkEngine3D.Mathematics.Vector3 v0;
            Vector3Ex.Negate(ref localTransformB.Position, out v0); //Since we're in A's local space, A-B is just -B.
            ...
            //This implementation is similar to that of the original XenoCollide.
            //'n' will be the direction used to find supports throughout the algorithm.
            OrkEngine3D.Mathematics.Vector3 n = rayDirection;
            OrkEngine3D.Mathematics.Vector3 v1;
            OrkEngine3D.Mathematics.Vector3 v1A, v1B; //extreme point contributions from each shape.  Used later to compute contact position; could be used to cache simplex too.
            MinkowskiToolbox.GetLocalMinkowskiExtremePoint(shapeA, shapeB, ref n, ref localTransformB, out v1A, out v1B, out v1);
            ...
            if (n.LengthSquared() < Toolbox.Epsilon)
            {
                ...
                float dot = Vector3Ex.Dot(v1 - minkowskiPosition, rayDirection);
                //Vector3Ex.Dot(ref v1, ref rayDirection, out dot);
                ...
                //Origin is inside.
                //Compute barycentric coordinates along simplex (segment).
                float dotv0 = Vector3Ex.Dot(v0 - minkowskiPosition, rayDirection);
                //Dot > 0, so dotv0 starts out negative.
                //Vector3Ex.Dot(ref v0, ref rayDirection, out dotv0);
                float barycentricCoordinate = -dotv0 / (dot - dotv0);
                //Vector3Ex.Subtract(ref v1A, ref v0A, out offset); //'v0a' is just the zero vector, so there's no need to calculate the offset.
                Vector3Ex.Multiply(ref v1A, barycentricCoordinate, out position);
                OrkEngine3D.Mathematics.Vector3 offset;
                Vector3Ex.Subtract(ref v1B, ref localTransformB.Position, out offset);
                return;
            }
            MinkowskiToolbox.GetLocalMinkowskiExtremePoint(shapeA, shapeB, ref n, ref localTransformB, out v2A, out v2B, out v2);

            OrkEngine3D.Mathematics.Vector3 temp;
            OrkEngine3D.Mathematics.Vector3 v0v1, v0v2;
            //Set n for the first iteration.
            Vector3Ex.Subtract(ref v1, ref v0, out v0v1);
            Vector3Ex.Subtract(ref v2, ref v0, out v0v2);
            Vector3Ex.Cross(ref v0v1, ref v0v2, out n);

            OrkEngine3D.Mathematics.Vector3 pointToV0;
            Vector3Ex.Subtract(ref v0, ref minkowskiPosition, out pointToV0);
            OrkEngine3D.Mathematics.Vector3 v3A, v3B, v3;
            int count = 0;
            while (true)
            {
                ...
                // If the origin is outside the plane defined by v1,v0,v3, then the portal is invalid.
                OrkEngine3D.Mathematics.Vector3 v0v3;
                Vector3Ex.Subtract(ref v1, ref v0, out v0v1);
                Vector3Ex.Subtract(ref v3, ref v0, out v0v3);
                Vector3Ex.Cross(ref v0v1, ref v0v3, out temp);
                float dot;
                Vector3Ex.Dot(ref temp, ref pointToV0, out dot);
                if (dot < 0)
                {
                    //Replace the point that was on the inside of the plane (v2) with the new extreme point.
                    v2 = v3;
                    ...
                    v2B = v3B;
                    // Calculate the normal of the plane that will be used to find a new extreme point.
                    Vector3Ex.Cross(ref v0v1, ref v0v3, out n);
                    continue;
                }

                // If the origin is outside the plane defined by v3,v0,v2, then the portal is invalid.
                Vector3Ex.Subtract(ref v2, ref v0, out v0v2);
                Vector3Ex.Cross(ref v0v3, ref v0v2, out temp);
                Vector3Ex.Dot(ref temp, ref pointToV0, out dot);
                if (dot < 0)
                {
                    //Replace the point that was on the inside of the plane (v1) with the new extreme point.
                    ...
                    v1B = v3B;
                    // Calculate the normal of the plane that will be used to find a new extreme point.
                    Vector3Ex.Cross(ref v0v2, ref v0v3, out n);
                    continue;
                }
                break;
            }
            ...
            while (true)
            {
                //Test the origin against the plane defined by v1, v2, v3.  If it's inside, we're done.
                //Compute the outward facing normal.
                OrkEngine3D.Mathematics.Vector3 v2v3, v2v1;
                Vector3Ex.Subtract(ref v3, ref v2, out v2v3);
                Vector3Ex.Subtract(ref v1, ref v2, out v2v1);
                Vector3Ex.Cross(ref v2v3, ref v2v1, out n);
                float dot;
                OrkEngine3D.Mathematics.Vector3 pointToV1;
                Vector3Ex.Subtract(ref v1, ref minkowskiPosition, out pointToV1);
                Vector3Ex.Dot(ref pointToV1, ref n, out dot);
                ...
                //    Vector3Ex.Cross(ref rayDirection, ref temp2, out cross);
                //    float v0ov2v3volume;
                //    Vector3Ex.Dot(ref cross, ref temp3, out v0ov2v3volume);

                //    Vector3Ex.Cross(ref temp1, ref rayDirection, out cross);
                //    float v0v1ov3volume;
                //    Vector3Ex.Dot(ref cross, ref temp3, out v0v1ov3volume);
                ...
                //If the origin is further along the direction than the extreme point, it's not inside the shape.
                float dot2;
                OrkEngine3D.Mathematics.Vector3 pointToV4;
                Vector3Ex.Subtract(ref v4, ref minkowskiPosition, out pointToV4);
                Vector3Ex.Dot(ref pointToV4, ref n, out dot2);
                ...
                if (dot2 - dot < rayCastSurfaceEpsilon || count > MPRToolbox.InnerIterationLimit) // TODO: Could use a dynamic epsilon for possibly better behavior.
                {
                    ...
                    Vector3Ex.Multiply(ref v1A, weight1, out position);
                    Vector3Ex.Multiply(ref v2A, weight2, out v2A);
                    Vector3Ex.Multiply(ref v3A, weight3, out v3A);
                    Vector3Ex.Add(ref v2A, ref position, out position);
                    Vector3Ex.Add(ref v3A, ref position, out position);
                ...
                Vector3Ex.Cross(ref pointToV4, ref pointToV0, out temp);
                Vector3Ex.Dot(ref pointToV1, ref temp, out dot);
                if (dot >= 0) //Dot v0 x v4 against v1, and dot it against 
                {
                    OrkEngine3D.Mathematics.Vector3 pointToV2;
                    Vector3Ex.Subtract(ref v2, ref minkowskiPosition, out pointToV2);
                    Vector3Ex.Dot(ref pointToV2, ref temp, out dot);
                    if (dot >= 0)
                    {
                        v1 = v4; // Inside v1 & inside v2 ==> eliminate v1
                        v1A = v4A;
                ...
                {
                    OrkEngine3D.Mathematics.Vector3 pointToV3;
                    Vector3Ex.Subtract(ref v3, ref minkowskiPosition, out pointToV3);
                    Vector3Ex.Dot(ref pointToV3, ref temp, out dot);
                    if (dot >= 0)
                    {
                        v2 = v4; // Outside v1 & inside v3 ==> eliminate v2
                        v2A = v4A;
        ...
        public static bool AreSweptShapesIntersecting(ConvexShape shapeA, ConvexShape shapeB, ref OrkEngine3D.Mathematics.Vector3 sweep, ref RigidTransform localTransformB, out OrkEngine3D.Mathematics.Vector3 position)
        {
            //It's possible that the two objects' centers are overlapping, or very very close to it.  In this case, 
            //they are obviously colliding and we can immediately exit.
            ...
            //This implementation is similar to that of the original XenoCollide.
            //'n' will be the direction used to find supports throughout the algorithm.
            OrkEngine3D.Mathematics.Vector3 n = localTransformB.Position;
            OrkEngine3D.Mathematics.Vector3 v1;
            OrkEngine3D.Mathematics.Vector3 v1A; //extreme point contributions from each shape.  Used later to compute contact position; could be used to cache simplex too.
            ...
            //Find another extreme point in a direction perpendicular to the previous.
            OrkEngine3D.Mathematics.Vector3 v2;
            OrkEngine3D.Mathematics.Vector3 v2A;
            Vector3Ex.Cross(ref v1, ref v0, out n);
            if (n.LengthSquared() < Toolbox.Epsilon)
            {
                //v1 and v0 could be parallel.
                ...
                //If the origin is within this extreme point, then there is an intersection.
                float dot;
                Vector3Ex.Dot(ref v1, ref localTransformB.Position, out dot);
                if (dot < 0)
                {
                    //Origin is outside.
                    position = new OrkEngine3D.Mathematics.Vector3();
                ...
                float dotv0;
                //Dot > 0, so dotv0 starts out negative.
                Vector3Ex.Dot(ref v0, ref localTransformB.Position, out dotv0);
                float barycentricCoordinate = -dotv0 / (dot - dotv0);
                //Vector3Ex.Subtract(ref v1A, ref v0A, out offset); //'v0a' is just the zero vector, so there's no need to calculate the offset.
                Vector3Ex.Multiply(ref v1A, barycentricCoordinate, out position);
                return true;
            ...
            OrkEngine3D.Mathematics.Vector3 v3A, v3;
            int count = 0;
            while (true)
            {
                //Find a final extreme point using the normal of the plane defined by v0, v1, v2.
            ...
            while (true)
            {
                ...
                if (dot >= 0)
                {
                    ...
                    Vector3Ex.Cross(ref localTransformB.Position, ref temp2, out cross);
                    float v0ov2v3volume;
                    Vector3Ex.Dot(ref cross, ref temp3, out v0ov2v3volume);

                    Vector3Ex.Cross(ref temp1, ref localTransformB.Position, out cross);
                    float v0v1ov3volume;
                    Vector3Ex.Dot(ref cross, ref temp3, out v0v1ov3volume);
        ...
        static void GetSweptExtremePoint(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform localTransformB, ref OrkEngine3D.Mathematics.Vector3 sweep, ref OrkEngine3D.Mathematics.Vector3 extremePointDirection, out OrkEngine3D.Mathematics.Vector3 extremePointA, out OrkEngine3D.Mathematics.Vector3 extremePoint)
        {
            OrkEngine3D.Mathematics.Vector3 b;
            ...
            float dot;
            Vector3Ex.Dot(ref extremePointDirection, ref sweep, out dot);
            if (dot > 0)
            {
                Vector3Ex.Add(ref extremePoint, ref sweep, out extremePoint);
            }
        }
        ...
        public static bool RayCast(Ray ray, float maximumLength, ConvexShape shape, ref RigidTransform transform, out RayHit hit)
        {
            ...
            Vector3Ex.Negate(ref localRay.Direction, out localRay.Direction);
            ...
            if (Toolbox.RayCastSphere(ref ray, ref transform.Position, shape.MaximumRadius, maximumLength, out sphereHit))
            {
                ...
                OrkEngine3D.Mathematics.Vector3 offset;
                Vector3Ex.Multiply(ref localRay.Direction, -sphereHit.T, out offset);
                Vector3Ex.Add(ref localRay.Position, ref offset, out localRay.Position);
            }
            else
            {
            ...
            if (rayLengthSquared > Toolbox.Epsilon * .01f)
            {
                Vector3Ex.Dot(ref localRay.Position, ref localRay.Direction, out sweepLength);
            ...
            OrkEngine3D.Mathematics.Vector3 sweep;
            Vector3Ex.Multiply(ref localRay.Direction, sweepLength, out sweep);
            //Check to see if the origin is contained within the swept shape.
            ...
            if (negativeLength)
            {
                ...
                hit.T = 0;
                Vector3Ex.Normalize(ref ray.Direction, out hit.Normal);
        ...
        public static bool SweptShapeContainsPoint(ConvexShape shape, ref OrkEngine3D.Mathematics.Vector3 sweep, ref OrkEngine3D.Mathematics.Vector3 localPoint)
        {
            //It's possible that the two objects' centers are overlapping, or very very close to it.  In this case, 
            //they are obviously colliding and we can immediately exit.
            ...
            OrkEngine3D.Mathematics.Vector3 v0;
            Vector3Ex.Negate(ref localPoint, out v0); //Since we're in A's local space, A-B is just -B.
            ...
            //This implementation is similar to that of the original XenoCollide.
            //'n' will be the direction used to find supports throughout the algorithm.
            OrkEngine3D.Mathematics.Vector3 n = localPoint;
            OrkEngine3D.Mathematics.Vector3 v1;
            //MinkowskiToolbox.GetLocalMinkowskiExtremePoint(shape, shapeB, ref n, ref localPoint, out v1A, out v1B, out v1);
            ...
            if (n.LengthSquared() < Toolbox.Epsilon)
            {
                ...
                //If the origin is within this extreme point, then there is an intersection.
                float dot;
                Vector3Ex.Dot(ref v1, ref localPoint, out dot);
                if (dot < 0)
                {
                    //Origin is outside.
                    return false;
        ...
        private static bool LocalSweepCast(ConvexShape shape, float sweepLength, float rayLengthSquared, ref OrkEngine3D.Mathematics.Vector3 localDirection, ref OrkEngine3D.Mathematics.Vector3 sweep, ref OrkEngine3D.Mathematics.Vector3 rayOrigin, out RayHit hit)
        {
            //By now, the ray is known to be within the swept shape and facing the right direction for a normal raycast.
        ...
        static void GetSweptExtremePoint(ConvexShape shape, ref OrkEngine3D.Mathematics.Vector3 point, ref OrkEngine3D.Mathematics.Vector3 sweep, ref OrkEngine3D.Mathematics.Vector3 extremePointDirection, out OrkEngine3D.Mathematics.Vector3 extremePoint)
        {
            shape.GetExtremePoint(extremePointDirection, ref Toolbox.RigidIdentity, out extremePoint);
            Vector3Ex.Subtract(ref extremePoint, ref point, out extremePoint);
            float dot;
            Vector3Ex.Dot(ref extremePointDirection, ref sweep, out dot);
            if (dot > 0)
            {
