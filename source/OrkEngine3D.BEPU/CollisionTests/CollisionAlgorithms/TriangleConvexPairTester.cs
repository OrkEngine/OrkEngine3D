...
namespace BEPUphysics.CollisionTests.CollisionAlgorithms
{
    ...
    public class TriangleConvexPairTester : TrianglePairTester
    {
        ...
        OrkEngine3D.Mathematics.Vector3 localSeparatingAxis;
        ...
        private bool DoPlaneTest(TriangleShape triangle, out TinyStructList<ContactData> contactList)
        {
            ...
            OrkEngine3D.Mathematics.Vector3 reverseNormal;
        ...
        private bool DoExternalNear(TriangleShape triangle, out TinyStructList<ContactData> contactList)
        {
            ...
            OrkEngine3D.Mathematics.Vector3 closestA, closestB;
            ...
            OrkEngine3D.Mathematics.Vector3 triangleCentroid;
            ...
            OrkEngine3D.Mathematics.Vector3 displacement;
            ...
            if (distanceSquared < margin * margin)
            {
                ...
                {
                    OrkEngine3D.Mathematics.Vector3 triangleNormal, ab, ac;
                    Vector3Ex.Subtract(ref triangle.vB, ref triangle.vA, out ab);
                    Vector3Ex.Subtract(ref triangle.vC, ref triangle.vA, out ac);
        ...
        private bool DoDeepContact(TriangleShape triangle, out TinyStructList<ContactData> contactList)
        {
            ...
            if (MPRToolbox.AreLocalShapesOverlapping(convex, triangle, ref center, ref Toolbox.RigidIdentity))
            {
                ...
                else
                {
                    ...
                    OrkEngine3D.Mathematics.Vector3 AO, BO, CO;
                    OrkEngine3D.Mathematics.Vector3 AB, BC, CA;
                    ...
                    OrkEngine3D.Mathematics.Vector3 ABnormal, BCnormal, CAnormal;
                    ...
                    if ((triangle.sidedness == TriangleSidedness.Clockwise && dot > 0) || (triangle.sidedness == TriangleSidedness.Counterclockwise && dot < 0))
                    {
                        ...
                        OrkEngine3D.Mathematics.Vector3 previousNormal = contact.Normal;
                        Vector3Ex.Dot(ref contact.Normal, ref triangleNormal, out dot);

                        OrkEngine3D.Mathematics.Vector3 p;
                        ...
                        else
                        {
                            ...
                            contact.Normal = new OrkEngine3D.Mathematics.Vector3();
                        }
                    }
                    ...
                    if ((triangle.sidedness == TriangleSidedness.Clockwise && dot > 0) || (triangle.sidedness == TriangleSidedness.Counterclockwise && dot < 0))
                    {
                        //Normal was facing the wrong way.
                        //Instead of ignoring it entirely, correct the direction to as close as it can get by removing any component parallel to the triangle normal.
                        OrkEngine3D.Mathematics.Vector3 previousNormal = candidateNormal;
        ...
        void TryToEscape(TriangleShape triangle, ref OrkEngine3D.Mathematics.Vector3 position)
        {
        ...
        private VoronoiRegion GetVoronoiRegion(TriangleShape triangle, ref OrkEngine3D.Mathematics.Vector3 p)
        {
            //The point we are comparing against the triangle is 0,0,0, so instead of storing an "A->P" vector,
            //just use -A.
            //Same for B->, C->P...

            OrkEngine3D.Mathematics.Vector3 ab, ac, ap;
            Vector3Ex.Subtract(ref triangle.vB, ref triangle.vA, out ab);
            Vector3Ex.Subtract(ref triangle.vC, ref triangle.vA, out ac);
            Vector3Ex.Subtract(ref p, ref triangle.vA, out ap);

            //Check to see if it's outside A.
            float APdotAB, APdotAC;
            Vector3Ex.Dot(ref ap, ref ab, out APdotAB);
            Vector3Ex.Dot(ref ap, ref ac, out APdotAC);
            ...
            Vector3Ex.Subtract(ref p, ref triangle.vB, out bp);
            Vector3Ex.Dot(ref ab, ref bp, out BPdotAB);
            Vector3Ex.Dot(ref ac, ref bp, out BPdotAC);
            ...
            Vector3Ex.Subtract(ref p, ref triangle.vC, out cp);
            Vector3Ex.Dot(ref ab, ref cp, out CPdotAB);
            Vector3Ex.Dot(ref ac, ref cp, out CPdotAC);
        ...
        public override void CleanUp()
        {
            ...
            state = CollisionState.Plane;
            escapeAttempts = 0;
            localSeparatingAxis = new OrkEngine3D.Mathematics.Vector3();
            Updated = false;
        }
        ...
        public override VoronoiRegion GetRegion(TriangleShape triangle, ref ContactData contact)
        {
            ...
            Vector3Ex.Dot(ref triangle.vA, ref contact.Normal, out dotA);
            Vector3Ex.Dot(ref triangle.vB, ref contact.Normal, out dotB);
            Vector3Ex.Dot(ref triangle.vC, ref contact.Normal, out dotC);
            ...
            if (dotA > dotB && dotA > dotC)
            {
                ...
                if (dotB > dotC)
                {
                    ...
                    {
                        Vector3Ex.Subtract(ref triangle.vB, ref triangle.vA, out edgeDirection);
                        Vector3Ex.Dot(ref edgeDirection, ref contact.Normal, out edgeDot);
                ...
                else
                {
                    ...
                    {
                        Vector3Ex.Subtract(ref triangle.vC, ref triangle.vA, out edgeDirection);
                        Vector3Ex.Dot(ref edgeDirection, ref contact.Normal, out edgeDot);
                        if (edgeDot * edgeDot < edgeDirection.LengthSquared() * edgeEpsilon)
                            return VoronoiRegion.AC;
                        else
            ...
            else if (dotB > dotC)
            {
                ...
                if (dotC > dotA)
                {
                    ...
                    {
                        Vector3Ex.Subtract(ref triangle.vC, ref triangle.vB, out edgeDirection);
                        Vector3Ex.Dot(ref edgeDirection, ref contact.Normal, out edgeDot);
                        if (edgeDot * edgeDot < edgeDirection.LengthSquared() * edgeEpsilon)
                            return VoronoiRegion.BC;
                        else
                ...
                else
                {
                    ...
                    {
                        Vector3Ex.Subtract(ref triangle.vA, ref triangle.vB, out edgeDirection);
                        Vector3Ex.Dot(ref edgeDirection, ref contact.Normal, out edgeDot);
                        if (edgeDot * edgeDot < edgeDirection.LengthSquared() * edgeEpsilon)
                            return VoronoiRegion.AB;
                        else
            ...
            else
            {
                ...
                if (dotA > dotB)
                {
                    ...
                    {
                        Vector3Ex.Subtract(ref triangle.vA, ref triangle.vC, out edgeDirection);
                        Vector3Ex.Dot(ref edgeDirection, ref contact.Normal, out edgeDot);
                        if (edgeDot * edgeDot < edgeDirection.LengthSquared() * edgeEpsilon)
                            return VoronoiRegion.AC;
                        else
                ...
                else
                {
                    ...
                    {
                        Vector3Ex.Subtract(ref triangle.vB, ref triangle.vC, out edgeDirection);
                        Vector3Ex.Dot(ref edgeDirection, ref contact.Normal, out edgeDot);
                        if (edgeDot * edgeDot < edgeDirection.LengthSquared() * edgeEpsilon)
                            return VoronoiRegion.BC;
                        else
