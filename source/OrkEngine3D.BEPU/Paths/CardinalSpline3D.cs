...
namespace BEPUphysics.Paths
{
    ...
    public class CardinalSpline3D : HermiteCurve3D
    {
        ...
        protected override void ComputeTangents()
        {
            ...
            {
                OrkEngine3D.Mathematics.Vector3 tangent;
                OrkEngine3D.Mathematics.Vector3 previous = ControlPoints[i - 1].Value;
                OrkEngine3D.Mathematics.Vector3 next = ControlPoints[i + 1].Value;
                Vector3Ex.Subtract(ref next, ref previous, out tangent);
                Vector3Ex.Multiply(ref tangent, (float)((1 - tension) / (ControlPoints[i + 1].Time - ControlPoints[i - 1].Time)), out tangent);
