...
namespace BEPUphysics.BroadPhaseSystems.SortAndSweep
{
    public class Grid2DSortAndSweepQueryAccelerator : IQueryAccelerator
    {
        ...
        public bool RayCast(Ray ray, float maximumLength, IList<BroadPhaseEntry> outputIntersections)
        {
            ...
            float length = 0;
            Int2 cellIndex;
            OrkEngine3D.Mathematics.Vector3 currentPosition = ray.Position;
            ...
            while (true)
            {
                ...
                Vector3Ex.Multiply(ref ray.Direction, nextT, out offset);
                Vector3Ex.Add(ref offset, ref currentPosition, out currentPosition);
                if (yIsMinimum)
                    if (ray.Direction.Y < 0)
                        cellIndex.Y -= 1;
                    else
        ...
        public void GetEntries(BoundingSphere boundingShape, IList<BroadPhaseEntry> overlaps)
        {
            ...
            //Loop through the cells and select bounding boxes which overlap the x axis.
#if !WINDOWS
            OrkEngine3D.Mathematics.Vector3 offset = new OrkEngine3D.Mathematics.Vector3();
#else
            OrkEngine3D.Mathematics.Vector3 offset;
#endif
