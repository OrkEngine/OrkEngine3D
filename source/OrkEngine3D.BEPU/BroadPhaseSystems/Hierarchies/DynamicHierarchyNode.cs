...
namespace BEPUphysics.BroadPhaseSystems.Hierarchies
{
    ...
    internal sealed class InternalNode : Node
    {
        ...
        internal override bool TryToInsert(LeafNode node, out Node treeNode)
        {
            ...
            Vector3Ex.Subtract(ref childA.BoundingBox.Max, ref childA.BoundingBox.Min, out offset);
            originalAVolume = offset.X * offset.Y * offset.Z;
            Vector3Ex.Subtract(ref childB.BoundingBox.Max, ref childB.BoundingBox.Min, out offset);
            originalBVolume = offset.X * offset.Y * offset.Z;

            float mergedAVolume, mergedBVolume;
            Vector3Ex.Subtract(ref mergedA.Max, ref mergedA.Min, out offset);
            mergedAVolume = offset.X * offset.Y * offset.Z;
            Vector3Ex.Subtract(ref mergedB.Max, ref mergedB.Min, out offset);
        ...
        void Reconstruct(RawList<LeafNode> leafNodes, int begin, int end)
        {
            ...
            OrkEngine3D.Mathematics.Vector3 offset;
            Vector3Ex.Subtract(ref BoundingBox.Max, ref BoundingBox.Min, out offset);
    ...
    internal sealed class LeafNode : Node
    {
        ...
        internal override bool TryToInsert(LeafNode node, out Node treeNode)
        {
            ...
            OrkEngine3D.Mathematics.Vector3 offset;
            Vector3Ex.Subtract(ref newTreeNode.BoundingBox.Max, ref newTreeNode.BoundingBox.Min, out offset);
