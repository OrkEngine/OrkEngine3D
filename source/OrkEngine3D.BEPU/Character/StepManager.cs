...
namespace BEPUphysics.Character
{
    ...
    public class StepManager
    {
        ...
        bool IsObstructiveToDownStepping(ref ContactData contact)
        {
            ...
            {
                float dot = Vector3Ex.Dot(contact.Normal, c.Contact.Normal);
        ...
        public bool TryToStepDown(out OrkEngine3D.Mathematics.Vector3 newPosition)
        {
            ...
            {
                newPosition = new OrkEngine3D.Mathematics.Vector3();
                return false;
            }
            ...
            OrkEngine3D.Mathematics.Vector3 normal = SupportFinder.SupportRayData.Value.HitData.Normal;
            ...
            OrkEngine3D.Mathematics.Vector3 intersection;
            Plane plane = new Plane(normal, Vector3Ex.Dot(SupportFinder.SupportRayData.Value.HitData.Location, normal));
            OrkEngine3D.Mathematics.Vector3 candidatePosition;
        ...
        CharacterContactPositionState TryDownStepPosition(ref OrkEngine3D.Mathematics.Vector3 position, ref OrkEngine3D.Mathematics.Vector3 down,
        ...
        public bool TryToStepUp(out OrkEngine3D.Mathematics.Vector3 newPosition)
        {
        ...
        void FindUpStepCandidates(RawList<ContactData> outputStepCandidates, ref OrkEngine3D.Mathematics.Vector3 down)
        {
            OrkEngine3D.Mathematics.Vector3 movementDirection = HorizontalMotionConstraint.MovementDirection3d;
            foreach (var c in SupportFinder.SideContacts)
            {
                ...
                float dot;
                Vector3Ex.Dot(ref contact.Normal, ref movementDirection, out dot);
                if (dot > 0)
                {
                    //It is! But is it low enough?
                    dot = Vector3Ex.Dot(down, c.Contact.Position - characterBody.Position);
                    ...
                    if (dot < characterBody.Height * .5f && dot > characterBody.Height * .5f - maximumStepHeight - upStepMargin)
                    {
                        ...
                        {
                            Vector3Ex.Dot(ref outputStepCandidates.Elements[i].Normal, ref contact.Normal, out dot);
                            if (dot > .99f)
                            {
        ...
        bool TryToStepUsingContact(ref ContactData contact, ref OrkEngine3D.Mathematics.Vector3 down, out OrkEngine3D.Mathematics.Vector3 newPosition)
        {
            OrkEngine3D.Mathematics.Vector3 position = characterBody.Position;
            ...
            OrkEngine3D.Mathematics.Vector3 normal = contact.Normal;
            float dot;
            ...
            OrkEngine3D.Mathematics.Vector3 horizontalOffset;
            ...
            OrkEngine3D.Mathematics.Vector3 supportNormal;
            ...
            OrkEngine3D.Mathematics.Vector3 verticalOffset;
        ...
        private void PrepareQueryObject(ref OrkEngine3D.Mathematics.Vector3 position)
        {
        ...
        CharacterContactPositionState TryUpStepPosition(ref OrkEngine3D.Mathematics.Vector3 sideNormal, ref OrkEngine3D.Mathematics.Vector3 position, ref OrkEngine3D.Mathematics.Vector3 down,
            ref QuickList<CharacterContact> tractionContacts, ref QuickList<CharacterContact> supportContacts, ref QuickList<CharacterContact> sideContacts, ref QuickList<CharacterContact> headContacts,
            out float hintOffset)
        {
            hintOffset = 0;
            ...
            if (headContacts.Count > 0)
            {
                ...
                float dot;
                Vector3Ex.Dot(ref down, ref headContacts.Elements[0].Contact.Normal, out dot);
                hintOffset = -dot * headContacts.Elements[0].Contact.PenetrationDepth;
                for (int i = 1; i < headContacts.Count; i++)
                {
                    Vector3Ex.Dot(ref down, ref headContacts.Elements[i].Contact.Normal, out dot);
            ...
            if (!obstructed && supportContacts.Count > 0)
            {
                ...
                if (supportState == CharacterContactPositionState.Accepted)
                {
                    ...
                    else
                    {
                        ...
                        float verticalOffset = Vector3Ex.Dot(downRay.Position - position, down);
                        ...
                        if (!QueryManager.RayCastHitAnything(obstructionTestRay, 1))
                        {
                            ...
                            if (QueryManager.RayCast(downRay, characterBody.Height, out hit))
                            {
                                ...
                                if (characterBody.Height - maximumStepHeight < hit.T)
                                {
                                    ...
                                    Vector3Ex.Dot(ref hit.Normal, ref down, out dot);
        ...
        bool IsUpStepObstructedBySideContacts(ref OrkEngine3D.Mathematics.Vector3 sideNormal, ref QuickList<CharacterContact> sideContacts)
        {
            //A contact is considered obstructive if its projected depth is deeper than any existing contact along the existing contacts' normals.
            for (int i = 0; i < sideContacts.Count; i++)
            {
        ...
        bool IsObstructiveToUpStepping(ref OrkEngine3D.Mathematics.Vector3 sideNormal, ref ContactData contact)
        {
