...
namespace BEPUphysics.Character
{
    ...
    public class CharacterContactCategorizer
    {
        ...
        public void CategorizeContacts<TOutputContacts>(CollidablePairHandler pair, EntityCollidable characterCollidable, ref OrkEngine3D.Mathematics.Vector3 downDirection, 
                                                            ...
                                                            where TOutputContacts : IList<CharacterContact>
        {
            ...
            for (int i = pair.ContactCount - 1; i >= 0; --i)
            {
                ...
                Vector3Ex.Subtract(ref characterContact.Contact.Position, ref characterCollidable.worldTransform.Position, out offset);
                Vector3Ex.Dot(ref characterContact.Contact.Normal, ref offset, out dot);
                if (dot < 0)
                {
                    //The normal should face outward.
                    Vector3Ex.Negate(ref characterContact.Contact.Normal, out characterContact.Contact.Normal);
                }

                Vector3Ex.Dot(ref characterContact.Contact.Normal, ref downDirection, out dot);
