...
namespace BEPUphysics.Character
{
    ...
    public class CharacterController : Updateable, IBeforeSolverUpdateable
    {
        ...
        public OrkEngine3D.Mathematics.Vector3 Down
        {
            ...
            set
            {
                ...
                if (lengthSquared < Toolbox.Epsilon)
                    ...
                    Vector3Ex.Divide(ref value, (float)Math.Sqrt(lengthSquared), out value);
                QuaternionEx.GetQuaternionBetweenNormalizedVectors(ref Toolbox.DownVector, ref value, out orientation);
                Body.Orientation = orientation;
            }
        }

        OrkEngine3D.Mathematics.Vector3 viewDirection = new OrkEngine3D.Mathematics.Vector3(0, 0, -1);
        ...
        /// </summary>
        public OrkEngine3D.Mathematics.Vector3 ViewDirection
        {
            get
            {
        ...
        public CharacterController(
            OrkEngine3D.Mathematics.Vector3 position = new OrkEngine3D.Mathematics.Vector3(),
        ...
        void ExpandBoundingBox()
        {
            if (Body.ActivityInformation.IsActive)
            {
                ...
                Vector3Ex.Multiply(ref down, StepManager.MaximumStepHeight, out expansion);
                ...
                OrkEngine3D.Mathematics.Vector3 squaredDown;
                ...
                Vector3Ex.Add(ref expansion, ref boundingBox.Max, out boundingBox.Max);
                Vector3Ex.Subtract(ref boundingBox.Min, ref expansion, out boundingBox.Min);
        ...
        void IBeforeSolverUpdateable.Update(float dt)
        {
            ...
            try
            {
                ...
                OrkEngine3D.Mathematics.Vector3 relativeVelocity;
                ComputeRelativeVelocity(ref supportData, out relativeVelocity);
                float verticalVelocity = Vector3Ex.Dot(supportData.Normal, relativeVelocity);
                ...
                if (tryToJump && StanceManager.CurrentStance != Stance.Crouching) //Jumping while crouching would be a bit silly.
                {
                    ...
                    if (SupportFinder.HasTraction)
                    {
                        ...
                        float currentDownVelocity = Vector3Ex.Dot(Down, relativeVelocity);
                    ...
                    else if (SupportFinder.HasSupport)
                    {
                        ...
                        float currentNormalVelocity = Vector3Ex.Dot(supportData.Normal, relativeVelocity);
                        //Target velocity is JumpSpeed.
                ...
                //Try to step!
                OrkEngine3D.Mathematics.Vector3 newPosition;
        ...
        SupportData TeleportToPosition(OrkEngine3D.Mathematics.Vector3 newPosition, float dt)
        {
        ...
        void CorrectContacts()
        {
            ...
            OrkEngine3D.Mathematics.Vector3 downDirection = Body.OrientationMatrix.Down;
            OrkEngine3D.Mathematics.Vector3 position = Body.Position;
            ...
            foreach (var pair in Body.CollisionInformation.Pairs)
            {
                foreach (var contactData in pair.Contacts)
                {
                    ...
                    OrkEngine3D.Mathematics.Vector3 offset = contact.Position - Body.Position;
                    ...
                    if (dot > minimumHeight)
                    {
                        ...
                        OrkEngine3D.Mathematics.Vector3 closestPointOnCylinder;
                        ...
                        OrkEngine3D.Mathematics.Vector3 offsetDirection;
        ...
        void ComputeRelativeVelocity(ref SupportData supportData, out OrkEngine3D.Mathematics.Vector3 relativeVelocity)
        {
            ...
            if (SupportFinder.HasSupport)
            {
                ...
                if (entityCollidable != null)
                {
                    ...
                    OrkEngine3D.Mathematics.Vector3 entityVelocity;
                    ...
                    Vector3Ex.Subtract(ref relativeVelocity, ref entityVelocity, out relativeVelocity);
                }
            }
        ...
        void ApplyJumpVelocity(ref SupportData supportData, OrkEngine3D.Mathematics.Vector3 velocityChange, ref OrkEngine3D.Mathematics.Vector3 relativeVelocity)
        {
            ...
            if (entityCollidable != null)
            {
                ...
                {
                    OrkEngine3D.Mathematics.Vector3 change = velocityChange * jumpForceFactor;
        ...
        public override void OnAdditionToSpace(Space newSpace)
        {
            ...
            Body.AngularVelocity = new OrkEngine3D.Mathematics.Vector3();
            Body.LinearVelocity = new OrkEngine3D.Mathematics.Vector3();
        }
